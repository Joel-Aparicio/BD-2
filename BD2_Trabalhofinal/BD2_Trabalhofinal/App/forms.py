...
class P_EquipaForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['clube'].label_from_instance = lambda obj: f"{obj.nome} (ID: {obj.get_id()})"
...
class P_CompeticaoForm(forms.ModelForm):
    ...
    class Meta:
        model = P_Competicao
        fields = ['nome', 'ano', 'imagem', 'formato', 'data_inicio', 'data_fim']
        widgets = {
            'nome': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Escreva o nome da Competição'}),
            'ano': forms.NumberInput(attrs={'class': 'form-control','placeholder': 'Ano da Competição','min': 0}),
            'imagem': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Insira o url da imagem'}),
        }
        labels = {
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['ano'].initial = 2000  # Define o valor padrão para 'ano'
